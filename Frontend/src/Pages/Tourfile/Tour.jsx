...
const Tour = ({ packageType: propPackageType, destinationIndex: propDestinationIndex }) => {
  ...
  useEffect(() => {
    ...
    if (pendingBooking) {
      try {
        ...
        // If package matches, auto-open payment form
        if (bookingData.selectedPackage) {
          setSelectedDestination(bookingData.selectedPackage);
          if (bookingData.selectedGuide) {
            setSelectedGuide(bookingData.selectedGuide);
          }
  ...
  // ✅ UPDATED: handleBookNow with authentication check
  const handleBookNow = async () => {
    // Check if user is logged in
    const isAuthenticated = authAPI.isAuthenticated();
    const user = authAPI.getStoredUser();

    console.log('🔐 Book Now clicked - Auth status:', { isAuthenticated, user });
    ...
    if (!isAuthenticated || !user) {
      ...
      const result = await Swal.fire({
        ...
        html: `
          <div style="text-align: center; padding: 1rem;">
            <p style="font-size: 1.1rem; margin-bottom: 1rem;">Please sign in to book this package.</p>
            <p style="color: #666; font-size: 0.95rem;">
              Don't have an account? You can create one in less than a minute!
            </p>
          </div>
        `,
      ...
      if (result.isConfirmed) {
        // Save package and guide data for after login
    ...
    // User is logged in - proceed with booking flow
